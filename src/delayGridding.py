...
def delaySq2Ps(vs,f,omegaP,band):
    '''
    ARGS:
    ...
    '''
    print('OmP=%.3f'%(omegaP))
    return littleh**3.*1e6*vs*X(f)**2.*Y(f)/(band*omegaP)*i2t(f,1)**2.

#just use this function to do everything
def delayTransformAndGrid(visList,bList,f0,df,nCells,uMax,flags,OmegaP=0.,dAnt=14.):
    '''
    ARGS:
    ...
    if OmegaP==0.:
        OmegaP=pi*(0.45*(c/f0)/dAnt)**2.
    return kperpAxis,kparaAxis,delaySq2Ps(ftV2Grid,f0,OmegaP,len(tAxis)*df)
